...
Реализовано 3 режима обучения: `supervised`, `rl` и `beam_rl`.

> - `supervised` - обучение с учителем по кросс-энтропии, включается при `--signal supervised`. В основной функции обучения в файле [train.py](./nps/train.py) есть соответствующие `if`-ы. Минибатчи для этой версии формируются в [data.py](./nps/data.py) в функции `get_minibatch`, обработка батча производится функцией `do_supervised_minibatch` или `do_syntax_weighted_minibatch` в файле [training_functions.py](./nps/training_functions.py).
>
> - `rl` - обучение по алгоритму REINFORCE c наградой, определяемой в файле [reinforce.py](./nps/reinforce.py). Включается при `--signal rl`. Обучение в том же файле [train.py](./nps/train.py) (только в других `if`-ах). Минибатчи формируются в [data.py](./nps/data.py) в функции `get_minibatch`, обработка батча производится функцией `do_rl_minibatch` (или `do_rl_minibatch_two_steps`) в файле [training_functions.py](./nps/training_functions.py).
>
> - `beam_rl` - обучение с подкреплением по программам, отобранным beam search, для каждого примера сохраняется луч кандидатов, затем им назначается награда через те же окружения и правила агрегации награды, что и в режиме `rl` ([reinforce.py](./nps/reinforce.py)). Включается при `--signal beam_rl`. Обучение в том же файле [train.py](./nps/train.py) (только в других `if`-ах). Минибатчи (вместе с мирами) формируются в [data.py](./nps/data.py) в функции `get_minibatch`; обработку батча производит функция `do_beam_rl` в файле [training_functions.py](./nps/training_functions.py).

 - каким образом реализована проверка правильности синтаксиса генерируемой программы?

Реализована двумя способами: `use_grammar` и `learn_syntax`.

- `use_grammar` - грамматика описывается в [checker.pyx](./syntax/checker.pyx) файле, когда модель генерирует распределение над токенами (логиты), применяется чекер и для недопустимых токенов к их логиту добавляется -бесконечность (`syntax_mask.masked_fill_(syntax_err_pos, -float('inf'))`)

- `learn_syntax` - появляется еще одна модель (`class SyntaxLSTM(nn.Module):`), обучается она так: из основного лосса вычитается взвешенный лосс LSTM `syntax_loss = -syntax_logit.gather(2, out_tgt_seq.unsqueeze(2)).sum()` на токенах, которые точно правильные (были встречены в эталонной программе). (учим модель на правильных токенах предсказывать нулевую поправку)

- `evaluation` - ко всем логитам прибавляется поправка: syntax_mask= -exp(s), на правильных токенах $s$ большой отрицательный - сумма не меняется, на неправильных $s$ - большой положительный $\Rightarrow$ отрицательная прибавка больше $\Rightarrow$ логиты приглушаются
...
> Это один шаг policy gradient: мы сэмплируем траектории из текущей политики, запускаем их в мир Karel, получаем награды, после чего с их помощью обновляем параметры модели, чтобы увеличить ожидаемую награду.

 - поясните различие между RL за два шага и за один шаг с разверткой (rollout)

> Разница в построении графа вычислений: в случае одного прохода, все делается в одном графе: сэмплирование, подсчет лог вероятностей токенов, подсчет наград, поэтому Pytorch может провести градиент назад по каждому токену. Это проще, но граф получается большим по памяти. В варианте за два шага сэмплирование происходит без тяжелого графа. Награды считаются как и ранее. Только после этого делается шаг по градиентам (`reinforce_grad`).
...
> При исходном коде время обучения растет линейно с размером датасета, потому что узкое горлышко (очень узкое) - это формирование батчей и загрузка их на GPU. Все это происходит синхронно с обработкой батча, в одном потоке и на чистом Python. Это очень долго, поэтому время вычислений на GPU пренебрежимо мало по сравнению с этим
>
> Итого: 1% ~ 1m, 3% ~3m, 10% ~ 10m при обучении supervised

 - убедитесь, что обученные промежуточные модели сохраняются в надежном месте по ходу обучения и объясните как это работает

> Сохраняется в папку `exps/*/Weights` веса начальной модели, модели с лучшим Accuracy и последней модели.
>
> Работает так:
>
> - в блоке после эпохе обучения, начинающемся с  `with open(str(path_to_weight_dump), "wb") as weight_file:` происходит обновление весов с последней эпохи
>
> - в блоке валидации, в случае, если достигнута лучшая точности `if val_acc > best_val_acc:`, происходит обновление весов лучшей модели
>
> - при обучении BeamRL в блоке логгирования `if signal == TrainSignal.BEAM_RL:` происходит обновление весов модели (так как здесь эпоха занимает намного больше времени)
>
...
> Примеры логов в папке [images](./images/)
...
Время обучения в supervised режиме (примерно одинаково для `use_grammar` и `learn_syntax`):
- 1% датасета: ~7s на эпоху, ~2m на валидацию, итого ~1h (V100)
- 3% датасета: ~20s на эпоху, ~2m на валидацию, итого ~2h (V100)
- 10% датасета: ~20s на эпоху, ~2m на валидацию, итого ~2h (A100)

Время обучения в beam_rl режиме (примерно одинаково для `use_grammar` и `learn_syntax`):
- 1% датасета: ~5m10s на эпоху, ~2m на валидацию, итого ~40m (V100)
- 3% датасета: ~22m на эпоху, ~2m на валидацию, итого ~2h (V100)
- 10% датасета: ~1h10m на эпоху, ~2m на валидацию, итого ~6h (A100)

Использовал несколько ВМ: Nvidia H100, Nvidia A100, Nvidia T4, Nvidia V100.

 - какого качества удалось достичь?

При обучении supervised на целом датасете удалось достичь ~61% точности. При обучении на Small dataset удалось достичь ~ 44% при обучении supervised и 47% при дальнейшем файн тьюнинге RL beam.

 - приведите 10 примеров синтеза программы для karel обученной моделью

> Примеры находятся в папке [examples](./result/examples/)
